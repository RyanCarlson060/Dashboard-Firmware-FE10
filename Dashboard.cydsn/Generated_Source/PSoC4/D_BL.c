...
#include "D_BL.h"
...
#if defined(D_BL__PC)
    #define D_BL_SetP4PinDriveMode(shift, mode)  \
    do { \
        D_BL_PC =   (D_BL_PC & \
                                (uint32)(~(uint32)(D_BL_DRIVE_MODE_IND_MASK << \
                                (D_BL_DRIVE_MODE_BITS * (shift))))) | \
                                (uint32)((uint32)(mode) << \
                                (D_BL_DRIVE_MODE_BITS * (shift))); \
    } while (0)
#else
        ...
        #define D_BL_SetP4PinDriveMode(shift, mode)  \
        do { \
            D_BL_USBIO_CTRL_REG = (D_BL_USBIO_CTRL_REG & \
                                    (uint32)(~(uint32)(D_BL_DRIVE_MODE_IND_MASK << \
                                    (D_BL_DRIVE_MODE_BITS * (shift))))) | \
                                    (uint32)((uint32)(mode) << \
                                    (D_BL_DRIVE_MODE_BITS * (shift))); \
    ...
    *******************************************************************************/
    ...
    {
		D_BL_SetP4PinDriveMode(D_BL__0__SHIFT, mode);
    }
#endif
...
{
    uint8 drVal = (uint8)(D_BL_DR & (uint8)(~D_BL_MASK));
    drVal = (drVal | ((uint8)(value << D_BL_SHIFT) & D_BL_MASK));
    D_BL_DR = (uint32)drVal;
}
...
{
    return (uint8)((D_BL_PS & D_BL_MASK) >> D_BL_SHIFT);
}
...
{
    return (uint8)((D_BL_DR & D_BL_MASK) >> D_BL_SHIFT);
}
...
void D_BL_SetInterruptMode(uint16 position, uint16 mode)
{
    ...
    intrCfg =  D_BL_INTCFG & (uint32)(~(uint32)position);
    D_BL_INTCFG = intrCfg | ((uint32)position & (uint32)mode);
}
...
{
	uint8 maskedStatus = (uint8)(D_BL_INTSTAT & D_BL_MASK);
	D_BL_INTSTAT = maskedStatus;
    return maskedStatus >> D_BL_SHIFT;
}
