...
{
    CyRamVectors[CYINT_IRQ_BASE + ReadyToDrive_Int__INTC_NUMBER] = address;
}
...
{
    return CyRamVectors[CYINT_IRQ_BASE + ReadyToDrive_Int__INTC_NUMBER];
}
...
void ReadyToDrive_Int_SetPriority(uint8 priority)
{
    ...
    uint32 priorityOffset = ((ReadyToDrive_Int__INTC_NUMBER % 4u) * 8u) + 6u;
    
	interruptState = CyEnterCriticalSection();
    *ReadyToDrive_Int_INTC_PRIOR = (*ReadyToDrive_Int_INTC_PRIOR & (uint32)(~(uint32)ReadyToDrive_Int__INTC_PRIOR_MASK)) |
...
uint8 ReadyToDrive_Int_GetPriority(void)
{
    ...
    priority = (*ReadyToDrive_Int_INTC_PRIOR & ReadyToDrive_Int__INTC_PRIOR_MASK) >> priorityOffset;
